...
class Trainer:
    ...
    def __init__(self,
        ...
        *,
        ...
        prefetch_data=True,
        model_version="Arti_stage2_flow",
        i_print=1000,
        i_log=500,
        i_sample=10000,
        i_save=10000,
        ...
        **kwargs
        ...
        if load_dir is not None and step is not None:
            # print("load from ...x")
            self.load_from_step = step
            self.load(load_dir, step)
        elif self.finetune_ckpt is not None: ### lora 和这个只能2选一
    ...
    def prepare_dataloader(self, **kwargs):
        ...
        ### print in red color
        # print("\033[91m""## YOU ARE USING 1 THREAD.""\033[0m".format(len(self.data_sampler)))
        # print("\033[91m""## YOU ARE USING 1 THREAD.""\033[0m".format(len(self.data_sampler)))
        configured_num_workers = kwargs.get('num_workers')
        if configured_num_workers is None:
            num_workers = max(int(np.ceil(os.cpu_count() / torch.cuda.device_count()) / 8), 4)
        else:
            num_workers = int(configured_num_workers)
            if num_workers < 0:
                raise ValueError(f"num_workers must be non-negative, got {num_workers}")
        print("\033[91m""## YOU ARE USING {} num_workers.""\033[0m".format(num_workers))
        ...
        self.dataloader = DataLoader(
            ...
            collate_fn=functools.partial(self.dataset.collate_fn, split_size=self.batch_split) if hasattr(self.dataset, 'collate_fn') else None,
    ...
    def snapshot_dataset(self, num_samples=4):
        ...
        if self.model_version=="Arti_stage2_flow":
            vis,videos = self.visualize_sample(data)
        else:
            videos = None
            vis,video = self.visualize_sample(data)
    ...
    @torch.no_grad()
    def snapshot(self, suffix=None, num_samples=10, batch_size=4, verbose=False, sample_steps=50, shuffle=True):
        """
        ...
        for key in list(samples.keys()):
            if samples[key]['type'] == 'sample':
                ...
                if videos is not None and self.is_master:
                    ...
                    os.makedirs(os.path.join(self.output_dir, 'samples', suffix, f'videos_{key}'), exist_ok=True)
    ...
    def run(self):
        ...
        while self.step < self.max_steps:
            ...
            if self.is_master:
                ...
                if self.step % self.i_log == 0:
                    ...
                    try:
                        ...
                        log_str = f'Error converting log to JSON: {e}'
                        print("json error:", e,"\n",log_str,)
                        print("log——info:", log)
            ...
            # Snapshot-based validation contains distributed collectives and
            # must therefore run on every rank. Only rank 0 writes metrics.
            if self.i_validate > 0 and self.step % self.i_validate == 0:
                metric_dict = self.validate()
                if self.is_master:
                    for key, value in metric_dict.items():
                        self.writer.add_scalar(f'{key}', value, self.step)
        ...
        # Snapshot methods gather across ranks, so every process participates.
