...
class ImageConditionedMixin:
    ...
    def _init_image_cond_model(self):
        ...
        with dist_utils.local_master_first():
            ...
            if not "dinov3" in self.image_cond_model_name:
                self.image_resolution = 518
                dino_model = torch.hub.load('facebookresearch/dinov2', self.image_cond_model_name, pretrained=True)
            else:
                raise ValueError(
                    "PAct training currently supports DINOv2 conditioning only; "
                    f"got {self.image_cond_model_name!r}."
                )
    ...
    def encode_image(self, image: Union[torch.Tensor, List[Image.Image], List[torch.Tensor]]) -> torch.Tensor:
        ...
        """
        # Synthetic smoke tests and cached-conditioning datasets may provide
        # normalized DINO tokens directly.
    ...
    def vis_cond_mask(self, cond, **kwargs):
        """
        Visualize the conditioning data.
        ...
        """
        if isinstance(cond, torch.Tensor):
            # cond = (cond - cond.amin(dim=[1,2],keepdim=True)) / (cond.amax(dim=[1,2],keepdim=True) - cond.amin(dim=[1,2],keepdim=True))
            ## unsqueeze from [B, H, W] to [B, 1, H, W]
            # cond = cond.unsqueeze(1)  # add channel dimension
            ## repeat from [B,1, H, W] to [B, 3, H, W]
            # cond = cond.repeat(1,3,1,1)
            # cond = cond.unsqueeze(1)  # add channel dimension
            return {'mask': {'value': cond, 'type': 'image'}}
        else:
            raise ValueError(f"Unsupported type of cond: {type(cond)}")
