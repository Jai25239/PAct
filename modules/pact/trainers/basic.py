...
class BasicTrainer(Trainer):
    ...
    def load(self, load_dir, step=0):
        ...
        try:
            if not os.path.exists(os.path.join(load_dir, 'ckpts', f'misc_step{step:07d}.pt')):
                raise FileNotFoundError(f"misc_step{step:07d}.pt not found") 
            ...
            if self.is_master:
                print(f'Warning: Failed to load miscellaneous checkpoint: {e}')
                print("LR is set to initial value.")
    ...
    def _unwrap(self, model):
        """
        Unwrap PEFT wrapper to underlying base model if present.
        Returns the original model if no known wrapper is detected.
        """
        try:
            from peft import PeftModel
            if isinstance(model, PeftModel):
                return getattr(model, 'model', getattr(model, 'base_model', model))
        except Exception:
            # peft not available or different version; just return as-is
            pass
        return model
    
    def finetune_from(self, finetune_ckpt, map_to_master_param=True):
        """
        ...
        for name, model in self.models.items():
            ...
            if name in finetune_ckpt:
                ...
                for k, v in model_ckpt.items():
                    # if self.kwargs.get('lora_setting', None) is not None and self.kwargs['lora_setting']["use_lora"]:
        ...
        if self.kwargs.get('lora_setting', None) is not None and self.kwargs['lora_setting']["use_lora"]:
            is_lora = True
            print("Using Lora, skip loading master params")
            pass 
        elif not map_to_master_param:
            pass
            is_lora = False
        else:
            is_lora = False
        ...
        # Verify that DDP is working correctly
        if map_to_master_param and not is_lora and self.world_size > 1:
            self.check_ddp()
    ...
    def run_step(self, data_list):
        ...
        # Logs
        step_log['loss'] = dict_reduce(losses, lambda x: float(np.mean(x)))
        step_log['status'] = dict_reduce(statuses, lambda x: float(np.mean(x)), special_func={'min': lambda x: np.min(x), 'max': lambda x: np.max(x)})
        if self.elastic_controller_config is not None:
